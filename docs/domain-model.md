...
implemented isolated read-only adapter outside the Domain. Slice 4 adds no new
Domain authority shape; it implements Runtime-owned non-Answer coordination,
source-bound Projection construction, deterministic Admission evaluation,
clarification, abandonment, non-Answer no-execution, and typed status views over
...
meanings. Its ProjectRead directive input is the complete decoded retained
...
bounded `PASS` on 2026-08-13. The assessment-owned budget policy and review
change no Domain authority.
...
The M2.6 Slice 1 Interaction records, strict codecs, canonical projections,
lifecycle and relationship invariants, and deterministic Runtime policy passed
...
Workflow, Evidence, or Acceptance authority. Slice 2's bounded SQLite
Interaction authority passed independent review on 2026-08-16. It implements
policy installation, atomic Session/Message/Focus and result-specific
Operation transactions, immutable Pending Action/Resolution/Reservation/
Outcome and Handoff authority, strict reopen, exact-project startup detection,
and terminal unresolved-work closure. Frontstage Context and Assistant
composition remain later M2.6 work.
...
Intent Projection has two closed record-schema variants. Schema version 1 is
paired with canonical profile `codeclosure-m2-5-projection-v1` and requires a
non-blank objective. The Slice 4 compiler emits schema version 2 paired with
`codeclosure-m2-5-projection-v2`; an absent objective is legal only as one exact
Proposal-digest-bound unresolved `OBJECTIVE_UNRESOLVED` ambiguity, with no
objective Source Binding or placeholder text. Admission and Store
commit/reopen enforce that closure; Goal Materialization continues to require
one non-blank `USER_STATED` objective. Migration
`0029_intent_projection_schema_v2.sql` preserves existing schema-version-1
records and digests byte-for-byte while admitting the new variant.
...
The phase `allowedRoots` are stable installed root envelopes, not
Attempt-specific snapshot or Candidate leaves. The exact ProjectRead record or
Candidate lease owns the dynamic workspace and cwd. Domain validation requires
normalized absolute non-overlapping phase roots and requires every phase to
forbid the shared controlled-state root. SQLite then cross-binds a v3
ProjectRead record's exact phase-entry digest, isolation, capability/response
contracts, workspace containment, and forbidden-root superset before atomic
Context/Attempt Start or external dispatch. The Adapter separately requires an
exact supported derived isolation digest; ProjectRead and Candidate execution
both use the union of phase and source-authority forbidden roots. Historical
external definitions and directive v2 retain their prior meaning.
...
no FakeWorker fallback. B4 composes that path through the trusted production
graph, and B5 closes its deterministic proof owners. The bounded Slice 3
`PASS` proves only the explicit protocol-fixture path; it does not establish
live Codex execution or effective live containment.
...
| Candidate stable change set — implemented and accepted in bounded M2.5.1 | Candidate Manager comparison of the persisted base manifest with equal stable source-freeze manifests | Workflow Runtime derives exact current Goal allowed-path disposition; Store recomputes canonical Profile/Candidate/digest/path/Check/Attempt relationships; SQLite guards retained shape, cross-record bindings, audit, and atomicity | Additive schema-version-2 freeze observation/Evidence in the existing atomic source-freeze transaction with Store-validated strict reopen; historical v1 remains separate and no Adapter or Worker gains containment, Evidence, or completion authority |
| Project-source read and Cleanup authority — implemented with deterministic and Live proof in bounded M2.5.1 | Goal scope, Candidate Manager observation, strict persisted project-read authority, Store-derived authority snapshot, exact cleanup records, and exact unresolved-Grant request | Workflow Runtime for project-read/Context/Attempt composition and the two exact currency checkpoints; trusted Runtime Cleanup coordinator for exact cleanup-request admission; Candidate Manager, Store canonical/relationship backstops, and Adapter-local physical classification | Store atomically persists the Runtime-authored project-read record plus Context/Attempt/Workflow/command/audits and exact integrity failure; the Store also persists the monotonic snapshot and consume-once Grant; the trusted Runtime Cleanup coordinator alone invokes the exact admitted port request; Store atomically closes Observation/Outcome/audit/consumption, and strict reopen/replay suppresses later filesystem work |
