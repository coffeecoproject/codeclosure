...
void test('[I-006][I-007][I-008] verified activation gates new Goal project admission', (t) => {
  ...
  assert.equal(store.appliedMigrations().length, 30);
...
void test('[I-006][I-008] migration cannot rewrite an inspected project binding', (t) => {
  ...
  writeFileSync(
    join(migrationsDirectory, '0031_rewrite_project_binding.sql'),
...
`,
  ...
  try {
    ...
    assert.equal(
      inspected.prepare('SELECT COUNT(*) FROM schema_migrations WHERE version = 31').pluck().get(),
      0,
    );
  } finally {
    inspected.close();
